...
\begin{demo}
    Por el teorema anterior, tomando $r=1$ y $x=0$, y por la relación de simetria \ref{simetria bernoulli}, obtenemos que $B_n = (-1)^n B_n(1) = (-1)^n B_n$, de donde se concluye.
\end{demo}
...
\begin{demo}
    ...
    \begin{equation*}
        \int_{x+ y}^{x} B_n^{(r)}(u) du = \left(\frac{\Delta_y}{T}\right) B_n^{(r)}(x) = \sum_{k=1}^{\infty} \frac{y^k}{k!} T^{k-1}B_{n}^{(r)}(x) 
    \end{equation*}
