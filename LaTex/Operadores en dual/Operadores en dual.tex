...
\begin{teorema}[caracterización de los operadores umbrales]
    Se tiene la igualdad de conjuntos $\mu_{\mathcal{C}}^{-1}(\Aut(\poly^*)) = \End_{umb}(\poly)$, es decir, un operador es un operador umbral si y solo si su operador adjunto es un automorfismo de $\poly^*$. Además, para todo operador umbral $\Lambda_F \in \End_{umb}(\poly)$ y todo funcional $G(T) \in \poly^*$ se verifica que
    \begin{equation*}
...
$\boxed{\mu_{\mathcal{C}}^{-1}(\Aut(\poly^*)) \subseteq \End_{umb}(\poly)}$ Sea $\alpha \in \Aut(\poly^*)$ un automorfismo. Por conservar los automorfismos el orden y ser biyecciones, habrá un único funcional delta $F(T) \in \poly^*$ tal que $\alpha F(T) = T$. En particular, por ser morfismo de álgebras será $\alpha F^k(T) = T^k$. Sea ahora $p_n(x)$ la sucesión asociada al funcional delta $F(T)$. Tenemos que
\begin{align*}
