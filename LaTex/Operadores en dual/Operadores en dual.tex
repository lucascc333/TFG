...
Comenzamos recordando que $\poly^*$ es un espacio topológico con la topología de estabilización \ref{topologia de estabilizacion} y que esta coincide con la topología formal, en virtud del teorema \ref{topologia formal equivalente a topologia de estabilizacion}. Esto nos permite enunciar y probar el siguiente teorema.

\begin{teorema} \label{equivalencia continuidad}
    Un operador lineal $\alpha \in \End(\poly^*)$ es continuo si y solo si verifica que para todas las sucesiones de funcionales $F_n(T)$ convergentes a 0 en la topología formal, la suma $\sum_{k=0}^\infty a_k \alpha F_k(T)$ es convergente y además
    \begin{equation*}
        \alpha \left( \sum_{k=0}^\infty a_k F_k(T) \right) = \sum_{k=0}^\infty a_k \alpha F_k(T)
    \end{equation*} 
...
\begin{demo}
    ...
    \begin{equation*}
        \left\langle \alpha \sum_{k=0}^\infty a_k F_k(T) \biggm| p(x) \right\rangle = \left\langle \alpha \sum_{k=0}^m a_k F_k(T) \biggm| p(x) \right\rangle + \left\langle \alpha \sum_{k=m+1}^\infty a_k F_k(T) \biggm| p(x) \right\rangle
    \end{equation*}
    Ahora, como la sucesión formada por las $F_n(T)$ es convergente se tiene que la sucesión $\{\sum_{k=m+1}^\infty a_k F_k(T)\}_{m \geq 0}$ también es convergente. Luego, como $\alpha$ es continuo sabemos que $\{ \alpha \sum_{k=m+1}^\infty a_k F_k(T)\}_{m \geq 0}$ es convergente y, por tanto, podemos tomar $m$ lo suficentemente grande como para que el segundo término se anule y $\ord(\alpha F_k(T)) > \gr(p(x))$ para todo entero $k > m$. Así pues, obtenemos la igualdad $\langle \alpha \sum_{k=0}^\infty a_k F_k(T) | p(x) \rangle = \langle \alpha \sum_{k=0}^m a_k F_k(T) | p(x) \rangle = \langle \sum_{k=0}^m a_k \alpha F_k(T) | p(x) \rangle = \langle \sum_{k=0}^\infty a_k \alpha F_k(T) | p(x) \rangle$, que es lo que buscábamos.
\end{demo}
...
Lo más importante en cuanto a estos nos conciernen es que los automorfismos de $\poly^*$ son todos continuos. Antes de probarlo, hemos de ver un par de resultados previos.

\begin{lema}
    Si $\alpha \in \Aut(\poly^*)$, entonces $\alpha F(T)$ es invertible (respecto al producto) si y solo si $F(T)$ es invertible (respecto al producto); es decir, $\ord(\alpha F(T)) = 0$ si y solo si $\ord(F) = 0$
\end{lema}

\begin{demo}
    Si $F(T)$ es invertible con inversa $1/F(T)$, entonces
    \begin{equation*}
    ...
    \end{equation*}
    Recíprocamente, si $\alpha F(T)$ es invertible con inversa $1/\alpha F(T)$ para el producto e inversa $\alpha^{-1}$ para la composición, entonces
    \begin{equation*}
    ...
    \end{equation*}
    Con lo que $\displaystyle \frac{1}{F(T)} = \alpha^{-1} \frac{1}{\alpha F(T)}$ y se concluye.
\end{demo}

\begin{teorema}
    Si $\alpha \in \Aut(\poly^*)$, entonces $\ord(\alpha F(T)) = \ord(F)$
    para toda $F(T) \in \poly^*$
\end{teorema}

\begin{demo}
    Veamos primero que $\ord(\alpha T) = 1$. Por el lema previo, $\ord(\alpha T) \geq 1$. Si fuera $\ord(\alpha T) > 1$, tendriamos que, para toda $F(T) \in \poly^*$
    \begin{equation*}
        F(T) = a_0 + T G(T) \implies \alpha F(T) = a_0 + \alpha (T \cdot G(T))
    \end{equation*}
    Ahora, $\alpha (T \cdot G(T)) = \alpha T \cdot \alpha G(T)$, por ser $\alpha$ un morfismo de álgebras, lo cual implica que $\ord(\alpha (T \cdot G(T))) \geq \ord(\alpha T) > 1$. Luego, $\ord(\alpha F(T)) \neq 1$ y no podría haber ninguna $F(T) \in \poly^*$ tal que $\alpha F(T) = T$, lo cual es imposible por la biyectividad de $\alpha$. Con lo que $\ord(\alpha T) = 1$ necesariamente. En consecuencia, como $\alpha T^k = (\alpha T)^k$, será $\ord(\alpha T^k) = k$.
...
Otra clase de operadores lineales sobre $\poly^*$ que son de especial relevancia son las derivaciones.
...
En particular, prestaremos atención a las derivaciones epiyectivas, cuyo conjunto lo escribiremos como $\Der_{\mathrm{epi}}(A)$. De forma similar a lo que ocurre con los automorfismos, su propiedad más importante es que todas son continuas. Para verlo, hemos de probar primero algunas de sus propiedades. 
...
\begin{teorema}
    Si $\delta \in \Der_{\mathrm{epi}}(\poly^*)$ y $F(T) \in \poly^*$ es tal que $\ord(F(T)) \geq 1$, entonces $\ord(\delta F(T)) = \ord(F)-1$
\end{teorema}
...
\begin{demo}
    ...
    \end{equation*}
    Como $\delta$ es epiyectiva, podemos escoger una $F(T)$ tal que $\ord(\delta F(T)) = 0$ y $\ord(G) > 0$. De tal manera que, como $\ord(T \cdot \delta G(T)) \geq 1$, necesariamente
    \begin{equation*}
        \ord(\delta T \cdot G(T)) = 0 \iff \ord(\delta T) + \ord (G) = 0 \iff \ord(\delta T) = 0
    \end{equation*}
    Si ahora consideramos una serie formal cualquiera $F(T) \in \poly^*$ con $\ord(F) = k$, podemos reescribirla como $F(T) = T^k G(T)$ con $\ord(G) = 0$. Luego, por \ref{derivaciones T^k} llegamos a que $\ord(\delta F(T)) = \ord(T^k \cdot \delta G(T) + kT^{k-1}G(T) \cdot \delta T) = k-1$, pues $\ord(kT^{k-1}G(T) \cdot \delta T) = k-1$ y $\ord(T^k \cdot \delta G(T)) = k$ debido a \ref{derivaciones nulas sobre k}.
\end{demo}

De esta manera, y como vimos con los automorfismos, queda probada la continuidad de esta clase de operadores.
...
\begin{definicion}
    Dado un operador $\Lambda \in \End(\poly)$, llamaremos \textbf{operador adjunto de $\Lambda$} al operador lineal $\Lambda^* \in \End(\poly^*)$ tal que para cada $F(T) \in \poly^*$, el operador $\Lambda^*F(T)$ se define como la aplicación dada por $p(x) \mapsto \langle F(T) | \Lambda p(x) \rangle$; es decir, es aquel tal que
    \begin{equation*}
        \langle \Lambda^* F(T) | p(x) \rangle = \langle F(T) | \Lambda p(x) \rangle
    \end{equation*}
\end{definicion}

\begin{proposicion}[propiedades del operador adjunto]
    Sean $\Lambda, \Omega \in \End(\poly)$ operadores cualesquiera y $\lambda \in \field$. Se verifican las siguientes propiedades.
...
\begin{proposicion}
    Para toda $F(T) \in \poly^*$ se verifica que el operador asociado $M^*(F)$ es un endomorfismo continuo. Además, si $F(T) \in \F^*$, entonces $M^*(F)$ es una biyección con inversa $M^*(F)^{-1} = M(1/F)$
\end{proposicion}

\begin{demo}
    Para la continuidad basta con aplicar el teorema \ref{equivalencia continuidad}. En el caso en el que la serie es invertible, $G(T) \xmapsto{M^*(F)} F(T) \cdot G(T) \xmapsto{M^*(1/F)} \frac{1}{F(T)} \cdot (F(T) \cdot G(T)) = G(T) $ y viceversa prueban que es una biyección con $M^*(F)^{-1} = M^*(1/F)$
\end{demo}
...
\begin{teorema}
    La aplicación adjunta restringida al espacio de los operadores lineales continuos de $\poly^*$, $\mu_{\mathcal{C}}$, es un isomorfismo con el espacio de operadores; es decir, se tiene que $\End(\poly) \backsimeq \End_{\mathcal{C}}(\poly^*)$, o lo que es lo mismo, que $\Im(\mu) = \End_{\mathcal{C}}(\poly^*)$ y $\mu$ es inyectiva.
\end{teorema}
\begin{demo}
    \begin{itemize}
        \item Inyectividad: Si $\Lambda^* = 0$, entonces $\langle F(T)| \Lambda p(x) \rangle = \langle \Lambda^* F(T)| p(x) \rangle = 0$ para toda serie formal $F(T)$ y todo polinomio $p(x)$. Con lo que $\Lambda = 0$ necesariamente.
        \item Epiyectividad: Sea $\alpha \in \End_{\mathcal{C}}(\poly^*)$. Dado un $n > 0$, por ser continuo, $\ord(\alpha T^k) > n$ para un $k$ suficientemente grande. Así pues, podemos considerar el operador $\Lambda_{\alpha}$ que viene definido por $\displaystyle \Lambda_{\alpha} x^n = \sum_{k=0}^\infty \frac{\langle \alpha T^k | x^n \rangle}{k!}x^k$. Este operador cumplirá que $\mu_{\mathcal{C}}(\Lambda_{\alpha}) = \Lambda_{\alpha}^* = \alpha$. En efecto:
        \begin{align*}
...
\begin{teorema}[caracterización de los operadores umbrales]
    Se tiene la igualdad de conjuntos $\mu_{\mathcal{C}}^{-1}(\Aut(\poly^*)) = \End_{\mathrm{Umb}}(\poly)$, es decir, un operador es un operador umbral si y solo si su operador adjunto es un automorfismo de $\poly^*$. Además, para todo operador umbral $\Lambda_F \in \End_{\mathrm{Umb}}(\poly)$ y todo funcional $G(T) \in \poly^*$ se verifica que
    \begin{equation*}
...
\begin{demo}
    ...
    $\boxed{\Aut(\poly^*) \supseteq \mu_{\mathcal{C}}(\End_{\mathrm{Umb}}(\poly))}$ Si la igualdad del enunciado se diera, entonces
    \begin{equation*}
        \Lambda_{F}^*(G(T)H(T)) = G(F^{-1}(T))H(F^{-1}(T)) = (\Lambda^*_{F}G(T))\cdot (\Lambda^*_{F}H(T)) 
    \end{equation*}
    y $\Lambda_{F}^*$ sería, por tanto, un automorfismo. Basta con ver entonces que se tiene dicha igualdad. Por las definiciones dadas tendremos que 
\begin{align*}
    \langle \Lambda_{F}^* F^k(T)| x^n\rangle =  \langle F^k(T) | \Lambda_{F} x^n\rangle & =  \langle F^k(T) | p_n(x)\rangle = n! \cdot \delta_{n,k} = \langle T^k | x^n \rangle
\end{align*}
Luego, $\Lambda_{F}^* F^k(T) =T^k$. Y por tanto, $\Lambda_{F}^*G(F(T)) = G(T)$ para toda $G(T) \in \poly^*$, ya que los operadores adjuntos son continuos. En particular, para $G(T) = (F^{-1}(T))^k$ tenemos que $\Lambda_{F}^*T^k = (F^{-1}(T))^k$, de donde se obtiene la igualdad del enunciado. 

$\boxed{\mu_{\mathcal{C}}^{-1}(\Aut(\poly^*)) \subseteq \End_{\mathrm{Umb}}(\poly)}$ Sea $\alpha \in \Aut(\poly^*)$ un automorfismo. Por conservar los automorfismos el orden y ser biyecciones, habrá un único funcional delta $F(T) \in \poly^*$ tal que $\alpha F(T) = T$. En particular, por ser morfismo de álgebras será $\alpha F^k(T) = T^k$. Sea ahora $p_n(x)$ la sucesión asociada al funcional delta $F(T)$. Tenemos que
\begin{align*}
...
\begin{teorema}[caracterización operadores de Sheffer]
    Sea $\Lambda \in \End(\poly)$ un operador cualquiera. $\Lambda$ es un operador de Sheffer asociado a $(G(T), F(T))$ si y solo si su operador adjunto $\Lambda^*$ tiene la forma
    \begin{equation*}
...
\begin{demo}
    $\boxed{\implies}$
    Se sigue directamente de la proposición anterior y las propiedades de los operadores adjuntos, de donde se deduce que
    \begin{equation*}
...
\begin{corolario}
    $\End_{\mathrm{Sh}}(\poly)$ es un grupo bajo la ley de grupo dada por la composición de aplicaciones. Concretamente $\Lambda_{G,F} \circ \Lambda_{H,L} = \Lambda_{G \cdot H \circ F, L \circ F} \text{ y } \Lambda_{G, F}^{-1} = \Lambda_{1/(G \circ F^{-1}), F^{-1}}$
\end{corolario}
...
\begin{demo}
    \begin{enumerate}
        ...
        \item $(\Lambda_G \circ \Lambda_{F^{-1}})p_n(x) = \Lambda_G x^n = q_n(x) \implies \Lambda = \Lambda_G \circ \Lambda_{F^{-1}} \in \End_{\mathrm{Umb}}(\poly)$. \qedhere 
    \end{enumerate}
\end{demo}
...
\begin{definicion}
    Sean $s_n(x) \sim (G(T), F(T))$ y $r_n(x) \sim (H(T), L(T))$ dos sucesiones de Sheffer. Se define la $\textbf{composición umbral}$ de $r_n(x) = \displaystyle \sum_{k=0}^n r_{n,k}x^k$ con $s_n(x)$ como el polinomio
    \begin{equation*}
...
\begin{teorema}[estructura algebraica de las sucesiones asociadas]
    $\Umb$ es un grupo \underline{no abeliano} con la ley de grupo dada por la composición umbral. En particular
    \begin{enumerate}
...
\begin{corolario}[estructura algebraica de las sucesiones de Sheffer]
    $\Sh$ es un grupo \underline{no abeliano} con la ley de grupo dada por la composición umbral. En particular
    \begin{enumerate}
...
\begin{demo}
    ...
    \begin{align*}
        \langle F(T) | [\Lambda_F^* G(T)] p(x) \rangle & = \langle F(T) \cdot \Lambda_F^*G(T)| p(x) \rangle = \langle \Lambda_F^* [G(T) \cdot (\Lambda_F^*)^{-1}F(T)]| p(x) \rangle \\
        & = \langle G(T) \cdot (\Lambda_F^*)^{-1}F(T) | \Lambda_F p(x) \rangle 
...
\begin{demo}
    Tenemos que $G(F^{-1}(T)) = \Lambda_F^* G(T) = \Lambda_{F^{-1}} \circ G(T) \circ \Lambda_F$. Utilizando que $(\Lambda_F^{-1})^{-1} = \Lambda_F$ se concluye.
\end{demo}
...
\begin{corolario}
    ...
    \begin{align*}
        \langle H(T) | s_n(x) \rangle = \left\langle \frac{1}{G(F^{-1}(T))}H(F^{-1}(T)) \biggm| x^n \right\rangle
    \end{align*}
\end{corolario}
...
\begin{definicion}
    ...
    \end{equation*}
    En el caso de que $p_n(x)$ sea la sucesión asociada a $F(T)$ decimos que $\theta_{1, F} \equiv \theta_{F}$ es simplemente la \textbf{traslación umbral asociada a $p_n(x) $ o a $F(T)$} y se simbolizarán a ambos conjuntos por $\TrasSh$ y $\TrasUmb$, respectivamente.
\end{definicion}
...
\begin{teorema}[caracterización de las traslaciones umbrales]
    Se tiene la igualdad de conjuntos $\mu_{\mathcal{C}}^{-1}(\Der_{epi}(\poly^*)) = \TrasUmb$, es decir, un operador es una traslación umbral si y solo si su operador adjunto es una derivación epiyectiva de $\poly^*$. Además, para toda traslación umbral $\theta_F \in \TrasUmb$ se verifica que $\theta_F^* = \partial_F$, es decir, es el operador de $\poly^*$ dado por
    \begin{equation*}
        \theta_F^*F^k(T) = kF^{k-1}(T)
    \end{equation*}
\end{teorema}

\begin{proposicion}\label{tras sheffer explicita}
    Dada $\theta_{G,F} \in \TrasSh$ se verifica que $\theta_{G,F} = \frac{1}{G(T)}\circ \theta_F \circ G(T)$
\end{proposicion}

\begin{teorema}[regla de la cadena]
    Dados $\theta_F, \theta_G \in \TrasUmb$ se verifica que
    \begin{equation*}
        \theta_F = \theta_G \circ \partial_F G(T)
    \end{equation*}
\end{teorema}

\begin{teorema}[fórmulas de recurrencia]
    Sea $p_n(x) \sim F(T)$. Se verifican
...
\begin{demo}
    \begin{enumerate}
        \item Si tomamos $G(T) = T$ se tiene que $\theta_G x^n = x^{n+1} \implies \theta_G = M$. Mediante el teorema anterior vemos que $\theta_F = M \circ \partial_F T = M \circ \frac{1}{\partial_T F(T)} = M \circ \frac{1}{F'(T)}$. Aplicándolo a $p_n(x)$ se obtiene la fórmula primera.

        \item Se ve usando \ref{conmutacion composicion operador umbral} y el apartado anterior
        \begin{align*}
            p_{n+1}(x) = M \circ \frac{1}{F'(T)} \circ \Lambda_F x^n & = M \circ \Lambda_F \circ \frac{1}{F'(F^{-1}(T))} x^n = M \circ \Lambda_F \circ (F^{-1}(T))' x^n \qedhere
        \end{align*}
    \end{enumerate}
\end{demo}

\begin{teorema}[fórmula de derivación]
    Dado $\theta_F \in \TrasUmb$, para todo operador $G(T) \in \End(\poly)$ se verifica que $\theta_F^* G(T) = G(T) \circ \theta_F - \theta_F \circ G(T)$. En particular
    \begin{equation*}
...
\begin{teorema}[fórmula de recurrencia para sucesiones de Sheffer]\label{recurrencia sheffer}
    Dada una traslación de Sheffer $\theta_{G,F} \in \TrasSh$, se verifica que $\theta_{G,F} = (M - \frac{G'(T)}{G(T)}) \circ \frac{1}{F'(T)}$
...
\begin{demo}
    Combinando la proposición \ref{tras sheffer explicita} y la igualdad $\theta_F = M \circ \frac{1}{F'(T)}$ vista en la demostración de la priemra de las fórmulas de recurrencia para las traslaciones umbrales, junto a la regla de la cadena y el teorema anterior, obtenemos el resultado
    \begin{align*}
        \theta_{G,F} & = M^*(1/G) \circ \theta_F \circ G(T) = M^*(1/G) \circ \left( G(T) \circ \theta_F - \theta_F^* \circ G(T) \right) \\
        & = \theta_F - M^*(1/G) \circ \partial_F \circ G(T) = \theta_F - M^*(1/G) \circ \partial_F T  \circ \partial_T G(T) \\
        & = M \circ \frac{1}{F'(T)} - M^*(1/G) \circ \frac{1}{F'(T)} \circ G'(T) = M \circ \frac{1}{F'(T)} - \frac{1}{G(T)} \left(\frac{1}{F'(T)}G'(T)\right)\\
        & = M \circ \frac{1}{F'(T)} - \frac{G'(T)}{G(T)} \circ \frac{1}{F'(T)} = \left(M - \frac{G'(T)}{G(T)} \right) \circ \frac{1}{F'(T)}\qedhere
    \end{align*}
\end{demo}
...
\begin{teorema}[fórmulas de transferencia]\label{transferencia asociada}
    Sea $p_n(x) \sim F(T)$. Se verifican las dos siguientes relaciones:
    \begin{enumerate}
        \item $\displaystyle p_n(x) = F'(T)\left(\frac{T}{F(T)}\right)^{n+1} x^n$
        \item $\displaystyle p_n(x) = M \circ \left(\frac{T}{F(T)}\right)^n x^{n-1}$
    \end{enumerate}
\end{teorema}
...
\begin{demo}
    ...
    \begin{align*}
        ...
        & = G^{-n}(T)x^n + nG'(T)G^{-n-1}(T)x^{n-1} \stackrel{(*)}{=} G^{-n}(T)x^n - (G^{-n}(T))'x^{n-1}\\
    ...
    Sea $n \geq 1$. Usando la igualdad (*) anterior, vemos que
    \begin{align*}
    ...
    \end{align*}
    donde hemos usado que el operador adjunto de $M$ es $\partial$. Si $n = 0$, entonces $\langle 1| q_0(x) \rangle = 0$ de forma evidente. Con lo que $\langle 1| q_n(x) \rangle = \delta_{n,0} $ y la primera condición se cumple. Así mismo
    \begin{align*}
...
\begin{teorema}[fórmula de inversión de Lagrange]
    Sean $F(T) \in \F^\delta$ y $G(T) \in \F^*$. Parar todo entero $n \geq 1$ se verifica que
    \begin{equation*}
        \langle G(F^{-1}(T)) | x^n \rangle = \left\langle G'(T)\left(\frac{T}{F(T)}\right)^{n} \biggm| x^{n-1} \right\rangle
    \end{equation*} 
    En particular, para $G(T) = T$ se tiene que
    \begin{equation*}
        \langle F^{-1}(T) | x^n \rangle = \left\langle \left(\frac{T}{F(T)}\right)^{n} \biggm| x^{n-1} \right\rangle
    \end{equation*}
\end{teorema}
...
\begin{demo}
    ...
    \begin{align*}
        ...
        & = \left\langle G(T) \biggm| M \circ \left(\frac{T}{F(T)}\right)^{n} x^{n-1} \right\rangle = \left\langle G'(T) \left(\frac{T}{F(T)}\right)^{n} \biggm| x^{n-1} \right\rangle
    \end{align*}
