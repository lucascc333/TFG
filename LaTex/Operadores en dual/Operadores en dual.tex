...
\begin{teorema}[caracterización operadors de Sheffer]
...
\begin{proposicion}\label{tras sheffer explicita}
    Dada $\theta_{G,F} \in \TrasSh$ se verifica que $\theta_{G,F} = M^*(1/G) \circ \theta_F \circ G(T)$
\end{proposicion}
...
\begin{teorema}[fórmula de derivación]
    Dado $\theta_F \in \TrasUmb$, para todo operador $G(T) \in \End(\poly)$ se verifica que $\theta_F^* G(T) = G(T) \circ \theta_F - \theta_F \circ G(T)$. En particular, se verifica
    \begin{equation*}
