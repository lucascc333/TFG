...
\begin{demo}
    ...
    \begin{equation*}
        (\alpha F(T))(\alpha \frac{1}{F(T)}) = \alpha(F(T) \frac{1}{F(T)}) = \alpha 1 = 1 \implies \frac{1}{\alpha F(T)} = \alpha \frac{1}{F(T)}
    \end{equation*}
    ...
    \begin{equation*}
        F(T) (\alpha^{-1} \frac{1}{\alpha F(T)}) = (\alpha^{-1}(\alpha F(T)))(\alpha^{-1} \frac{1}{\alpha F(T)}) = \alpha^{-1}(\alpha F(T) \frac{1}{\alpha F(T)}) = \alpha^{-1}1 = 1
    \end{equation*}
...
\begin{demo}
    ...
    \end{equation*}
    Ahora, $\alpha (T \cdot G(T)) = \alpha T \cdot \alpha G(T)$, por ser $\alpha$ un morfismo de álgebras, lo cual implica que $\ord(\alpha (T \cdot G(T))) \geq \ord(\alpha T) > 1$. Luego, $\ord(\alpha F(T)) \neq 1$ y no podría haber ninguna $F(T) \in \poly^*$ tal que $\alpha F(T) = T$, lo cual es imposible por la biyectividad de $\alpha$. Con lo que $\ord(\alpha T) = 1$ necesariamente. Así pues, para $k \geq 0$
    \begin{equation*}
...
\begin{demo}
    Para $k = 1$ la igualdad es cierta de forma evidente. Usando inducción, para $k+1$ tendremos
        \begin{equation*}
            \delta T^{k+1} = \delta T^k \cdot T + T^k \cdot \delta T =  k T^{k} \cdot \delta T + T^k \cdot \delta T = (k+1) T^k \cdot \delta T \qedhere
        \end{equation*}
\end{demo}
...
\begin{teorema}\label{derivaciones epi implica continuo}
    ...
    \begin{center}
        $\Der_{\mathrm{epi}}(\poly^*) \subset \End_{\mathcal{C}}(\poly^*)$
    \end{center}
\end{teorema}
...
\begin{demo}
    ...
    \begin{equation*}
        \Lambda_{G,F}x^n = s_n(x) = \frac{1}{G(T)} p_n(x) = (\frac{1}{G(T)} \circ \Lambda_{F}) x^n \implies \Lambda_{G,F} = \frac{1}{G(T)} \circ \Lambda_{F} \qedhere
    \end{equation*}
\end{demo}
...
\begin{demo}
    ...
    \begin{equation*}
        \Lambda_{G,F}^*H(T) = [\Lambda_{F}^* \circ M^*(1/G)]H(T) = \Lambda_{F}^*(\frac{1}{G(T)} \cdot H(T)) = \frac{1}{G(F^{-1}(T))} \cdot H(F^{-1}(T)) 
    \end{equation*}
...
\begin{teorema}
    \textbf{(fórmula de derivación)} Dado $\theta_F \in \TrasUmb$, para todo $G(T) \in \End(\poly)$ se verifica que $\theta_F^* G(T) = G(T) \circ \theta_F - \theta_F \circ G(T)$. En particular, se verifica que
    \begin{equation*}
...
\begin{demo}
    ...
    \begin{align*}
        ...
        & = \langle G^{-n}(T) | x^n \rangle - \langle \partial G^{-n}(T) | x^{n-1} \rangle = \langle G^{-n}(T) | x^n \rangle - \langle G^{-n}(T) | x^{n} \rangle = 0
    \end{align*}
...
\begin{demo}
    ...
    \begin{align*}
        ...
        & = \langle G(T) | M \circ (\frac{T}{F(T)})^{n} x^{n-1} \rangle = \langle G'(T) (\frac{T}{F(T)})^{n}| x^{n-1} \rangle
    \end{align*}
