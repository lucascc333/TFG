...
\usepackage{enumitem} % Para enumerar casos
    \setlist[itemize]{
    leftmargin=*,       % sin sangría extra
    topsep=4pt,
    itemsep=2pt,
    before=\leavevmode  % evita que el primer ítem quede suelto arriba
    }
...
\usepackage{multicol} % Para mayor control sobre enumeraciones
...
%: Firma alumno
\includepdf[pages=-]{firma_alumno.pdf}
...
%: Certificado tutor
\includepdf[pages=-]{certificadotutores_firmado 1.pdf}
...
\begin{flushright}
    ...
    \\\vspace*{1cm}
    \small\textit{Lo maravilloso es siempre bello, \\todo lo maravilloso es bello, \\de hecho, sólo lo maravilloso es bello.}
    \\[1em]\textsc{André Bretón}
    \\\vspace*{1cm}
    \small\textit{The pendulum of mathematics swings back and forth \\
    towards abstraction and away from it\\
    with a timing that remains to be estimated.}
    \\[1em]\textsc{Gian-Carlo Rota}
    \\\vspace*{1cm}
    \small\textit{Basta con que la parte visible esté impecable 
    \\para que se tenga una opinión favorable de la que no se ve.}
    \\[1em]\textsc{Junichirô Tanizaki}
\end{flushright}
...
\include{Algebra umbral/Algebra Umbral}

\include{Sucesiones de Sheffer/Sucesiones de Sheffer}

\include{Operadores en dual/Operadores en dual}

\include{Aplicaciones/Aplicaciones}

%: Apéndices
\include{Apéndices/Series formales}
