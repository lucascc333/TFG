...
\begin{itemize}
    ...
    \begin{equation*}
        ...
        \end{dcases}
    \end{equation*}
    \item $(x)_n$ simbolizará al factorial descendente. Esto es
    \begin{equation*}
...
Tal y como indica el título de la sección, nos centraremos en el estudio de los funcionales lineales sobre el espacio de polinomios $\poly$; es decir, las aplicaciones lineales $L$
\begin{align*}
...
\end{teorema}
\begin{demo}
    Veamos primero que es biyectiva.
        \begin{equation*}
        ...
        La linealidad es inmediata pues
        \begin{align*}
...
\begin{enumerate}
    ...
    \end{equation}
    Siguiendo la notación formal, simbolizaremos a esta serie por $E_\lambda (T) \equiv e^{\lambda T}$ y en caso de que $\lambda = 0$, se dice que $E_0$ es el \textbf{funcional aumento}. 
    
    Dicho esto, la proposición \ref{identidad algebra umbral} se puede reformular diciendo que el funcional aumento $E_0$ es la identidad para el producto del álgebra umbral.
...
\end{proposicion}
\begin{demo}
    Esto es consecuencia de la definición \ref{omegaF}, pues \begin{equation*}
    \langle F(T)| x^n\rangle = n! [F]_n \implies \langle F(T)| p(x)\rangle = \sum_{n}n![F]_n[p]_n 
    \end{equation*}
    y como $\ord(F(T)) > \gr(p(x))$, o bien $[F]_n = 0$ o bien $[p]_n = 0$.   
\end{demo}
...
\begin{demo}
    ...
    \end{equation*}
    Para un polinomio genérico $p(x)$, por la proposición anterior y la linealidad, escribiendo $m = \gr(p)$ se verifica que
    \begin{equation*}
...
\begin{demo}
    $F(T) = \sum_k a_kT^k$. Por la linealidad, basta verlo para $p(x) = x^n$. En efecto 
        $$\begin{aligned}
...
\begin{ej}
    El funcional $\frac{e^{\lambda T} -1}{T} = \frac{\Delta_\lambda}{T}$ (bien definido ya que $\ord(e^{\lambda T} - 1) = 1$) será aquel con
    \begin{equation*}
    ...
    \end{equation}
    Este funcional nos aparecerá cuando estudiemos los polinomios de Bernoulli y allí lo analizaremos con detenimiento. 
\end{ej}

Dos familias de funcionales son de especial importancia: estos son los \textit{funcionales delta} y los \textit{funcionales invertibles}. Los utilizaremos, fundamentalmente, cuando hablemos de las \textit{sucesiones de Sheffer}. Damos tanto sus definiciones como su caracterización.
...
\begin{obs}
    De forma similar a lo que hacíamos con los funcionales, a los operadores lineales de $\End(\poly)$ los llamaremos simplemente \textbf{operadores}. Recordamos también que el espacio de operadores, $\End(\poly)$,  tiene estructura de $\field$-álgebra, siendo el producto de operadores el producto usual.
\end{obs}
...
\begin{proposicion}
    Si $\{ F_k(T) \}_{k \geq 0}$ es un sistema de generadores topológico para $\F$, entonces la aplicación
    \begin{align*}
        \{F(T)\} \colon & \poly \longrightarrow \field^\infty\\
        &p(x) \mapsto \{ F_k(T)p(x)\}_{k \geq 0}
    \end{align*}
    que asocia a cada polinomio la sucesión $\{ F_k(T)p(x)\}_{k \geq 0}$, es inyectiva.
\end{proposicion}

\begin{proposicion}
    Si $\{ p_k(x) \}_{k \geq 0}$ es un sistema de generadores algebraico para $\poly$, entonces la aplicación
    \begin{align*}
        \{p(x)\} \colon & \poly \longrightarrow \field^\infty\\
        &p(x) \mapsto \{ F(T)p_k(x)\}_{k \geq 0}
    \end{align*}
    que asocia a cada serie formal la sucesión $\{ F(T)p_k(x)\}_{k \geq 0}$, es inyectiva.
\end{proposicion}
...
\end{corolario}
Si bien la asignación \eqref{T como derivada} es biunívoca de forma trivial, a diferencia de lo que ocurría con el i.c.u, la aplicación $\varphi$ \underline{\textbf{no}} es una aplicación biyectiva; es decir, existen operadores que no provienen de ninguna serie formal. Esto es consecuencia de que, como $\gr(\partial p) \leq \gr(p)$, por linealidad se tiene que
\begin{equation*}
    \gr(F(T)p(x)) \leq \gr(p)
\end{equation*}
...
\end{lema}
\begin{demo}
    Como venimos haciendo, por linealidad, basta con probarlo para los polinomios $x^n$. Por contradicción, supongamos que existe un número natural $m$ tal que $\gr(\Lambda x^n) \geq m+1$. Como $\ord(F) = 1$, tenemos que, siendo $F(T) = \sum_{k \geq 1} \frac{a_k}{k!}T^k$
    \begin{align*}
        F(T)x^n & = \sum_{k=1}^n \binom{n}{k}a_k T^k = n a_1 x^{n-1} + \dots + a_n, \quad \forall n \geq 1\\
        F(T)x^{0} & = 0
    \end{align*} 
    Por tanto, $\gr(F(T)p(x)) = \gr(p) - 1$ si $\gr(p) \geq 1$. Luego, de aquí deducimos que, como $\ord(F^{m+1}) = m+1$, entonces $F^{m+1}(T)x^m = 0$. Y, en consecuencia
    \begin{equation*}
