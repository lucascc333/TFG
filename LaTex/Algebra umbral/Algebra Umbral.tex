...
\begin{proposicion}
    Sea $F(T) \in \F$ y $p(x) \in \poly$ cualesquiera. Entonces, se dan las igualdades:
    \begin{enumerate}
        \item $\begin{aligned}
...
\begin{demo}
    $F(T) = \sum_k a_kT^k$. Por la linealidad, basta verlo para $p(x) = x^n$
    \begin{enumerate}
        \item Tenemos que: 
            $$\begin{aligned}
        ...
        \end{aligned}$$
        \item $T \cdot F(T) = \sum_{n=0}^\infty a_nT^{n+1} = \sum_{n=1}^\infty a_{n-1}T^n$. Similar a lo hecho antes:
        \begin{align*}
            \langle F(T) | (x^n)' \rangle & = \langle F(T) | nx^{n-1} \rangle = n\langle F(T) | x^{n-1} \rangle \\
            & = n(n-1)!a_{n-1} = n! a_{n-1} \\
            & = n![T \cdot F(T)]_n = \langle T \cdot F(T) | x^{n} \rangle
        \end{align*}
    \end{enumerate}
\end{demo}
    
\begin{corolario}
    Dados $F(T) \in \F$ y $p(x) \in \poly$ cualesquiera, se verifica:
    \begin{equation*}
...
\begin{proposicion}
    ...
    \begin{enumerate}
        \item $F(T)$ es un funcional delta si y solo si $\langle F(T)| 1\rangle = 0$ y $\langle F(T)| x \rangle \neq 0$
        \item $F(T)$ es un funcional invertible si y solo si $\langle F(T)|1 \rangle \neq 0$
    \end{enumerate}
\end{proposicion}
...
\begin{equation} \label{serie como operador}
    F(T)x^n = \sum_k \binom{n}{k}a_kx^{n-k}
\end{equation}
...
\begin{equation*}
    p(x) \mapsto xp(x)
\end{equation*}
es un ejemplo de un operador que no proviene de ninguna serie formal. Queda así probada la siguiente proposición.
