...
\begin{proposicion}\label{taylor analogo}
    ...
    \begin{enumerate}
        \item $\begin{aligned}[t]
            F(T) = \sum_{k = 0}^\infty \frac{ \langle F(T) | x^k \rangle}{k!}T^k \quad \textnormal{\textbf{(desarrollo de Taylor para funcionales)}}
        \end{aligned}$ 
        \item $\begin{aligned}
...
\begin{demo}
    ...
    \begin{align*}
        ...
        & = \sum_{k=0}^m a_k\left(\sum_{n=0}^m b_n\langle (\lambda T)^n| x^k\rangle \right) = \langle F(\lambda T) | p(x)\rangle \qedhere
    \end{align*}
\end{demo}
