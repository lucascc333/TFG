...
Nótese que hemos fijado la notación $\langle L | p(x) \rangle$ para simbolizar la acción de $L$ sobre $p(x)$, típica de la física. Más tarde se esclarecerá con detalle el motivo de esta elección. 
...
\begin{demo}
    ...
    \end{align*}
    Y, por tanto, son linealmente independientes con lo que se concluye.
\end{demo}
...
\begin{teorema} \textbf{(isomorfismo umbral)} La aplicación $\phi$ dada por
    \begin{align*}
        \phi \colon & \poly^* \xrightarrow{\sim} \F\\
        & L \mapsto F_L(T) = \sum_{n=0}^\infty \frac{ \langle L | x^n \rangle}{n!} T^n \\
        & \omega_F \mapsfrom F(T)
    \end{align*}
    donde el funcional $\omega_F$ viene dado por $\langle \omega_F | x^n \rangle = n! [F]_n$ es un isomorfismo lineal. A este morfismo $\phi$ lo llamaremos el \textbf{isomorfismo canónico umbral}.
\end{teorema}
\begin{demo}
    \begin{itemize}
        ...
        \begin{align*}
            \phi (L+cM) & = F_{L+M} (T) = \sum_{n=0} ^\infty \frac{\langle L + cM | p(x) \rangle}{n!}T^n = \sum_{n=0} ^\infty \frac{\langle L | p(x) \rangle}{n!}T^n + c\sum_{n=0} ^\infty \frac{\langle M | p(x) \rangle}{n!}T^n \\ &= F_L(T) + c\cdot F_M(T) = \phi (L) + c \cdot \phi (M)
        \end{align*}
    \end{itemize}
\end{demo}
...
\end{equation*}
Ahora bien, también es cierto que este isomorfismo induce una estructura de $\field$-álgebra en $\poly^*$ distinta de la que un principio teníamos y para la cual $\phi$ sí será un isomorfismo de álgebras de forma evidente. Esta es la que llamaremos \textit{álgebra umbral}.
...
\begin{corolario}
    El isomorfismo canónico umbral $\phi$ es un isomorfismo entre el álgebra umbral y el álgebra de series formales.
\end{corolario}
...
\begin{enumerate}
    ...
    \end{equation}
    Simbolizaremos a este funcional por $\varepsilon_\lambda \equiv e^{\lambda T}$ y en caso de que $\lambda = 0$, se dice que $\varepsilon_0$ es el \textbf{funcional aumento}. 
...
Mostramos aquí las propiedades fundamentales de los funcionales lineales sobre $\poly$, consecuencias todas ellas, más o menos directas, de su interpretación como series formales.
...
\begin{proposicion}\label{taylor analogo}
    ...
    \begin{enumerate}
        \item $\begin{aligned}[t]
            F(T) = \sum_{n \geq 0} \frac{ \langle F(T) | x^n \rangle}{n!}T^n
        \end{aligned}$ 
        \item $\begin{aligned}
            p(x) = \sum_{n \geq 0} \frac{ \langle T^n| p(x) \rangle}{n!}x^n
        \end{aligned}$
    \end{enumerate}
\end{proposicion}
...
\begin{demo}
    ...
    \end{equation*}
    Por la proposición anterior y la linealidad, siendo $m = \gr(p(x))$:
    \begin{equation*}
...
\begin{proposicion}
    Sea $\{p_k(x)\}_{k \geq 0}$ familia de polinomios tal que genera $\poly$. Entonces, la aplicación:
    \begin{align*}
...
\end{proposicion}
\begin{demo}
    Análoga a la anterior, escribiendo $x^n$ como combinación lineal de los $p_k(x)$. 
\end{demo}
...
Dos familias de funcionales son de especial importancia: estos son los \textit{funcionales delta} y los \textit{funcionales invertibles}. Concluimos la sección dando tanto sus definiciones como su caracterización.

\begin{definicion}
    Dada $F(T) \in \F$, diremos que $F(T)$ es una \textbf{serie delta} (o \textbf{$\delta$-serie}) si $F(T) \in \maximal$ y $F(T) \not \in \maximal^2$; es decir, cuando $F(T)$ es un invertible respecto de la composición. Escribimos $\F^\delta$ para simbolizar el conjunto de funcionales delta.
\end{definicion}

\begin{definicion}
    Dada un funcional $L \in \poly^*$, diremos que $L$ es un \textbf{funcional delta} (o \textbf{$\delta$-funcional}) si al pensarlo como una serie (es decir, $\phi(L)$) es una serie delta. Escribimos Análogamente, diremos que $L$ es un \textbf{funcional invertible} si al pensar $L$ como una serie formal esta es una serie invertible.
\end{definicion}
...
%: OPERADORES LINEALES
\section{Series como operadores lineales}
...
\begin{obs}
    De forma similar a lo que hacíamos con los funcionales, a los operadores lineales de $\End(\poly)$ los llamaremos simplemente \textbf{operadores}.
\end{obs}
...
\begin{align*}
    ...
    & \sum_{n\geq0} \frac{a_n}{n!}T^n \mapsto \sum_{n \geq 0} \frac{a_n}{n!}\partial^n
\end{align*}
...
Los operadores lineales $F(T)$ satisfarán propiedades análogas a la de los funcionales. Listamos algunas de estas aunque no damos su demostración puesto que son muy similares a las dadas para los funcionales.
...
\begin{proposicion}
    ...
    \end{equation*}
    para todo polinomio $p(x) \in \poly$
\end{proposicion}
...
Uno de los resultados claves en lo que a estas operadores concierne es el de la relación de adjunción que guardan respecto al producto formal.
...
\begin{corolario}
    Sea $F(T) \in \F$ y $p(x) \in \poly$ cualesquiera. Entonces, se verifica que
       ...
       \end{aligned}$$
    Equivalentemente, se verifica que
    \begin{equation*}
        \langle F(T) | p(x)\rangle = \langle T\cdot F(T)| \int p(x)\rangle
    \end{equation*}
\end{corolario}

\begin{corolario}
    Dada $F(T) \in \F$ y $p(x) \in \poly$ se tiene que
    \begin{equation*}
        \langle  F(T) | p(x) \rangle = (F(T)p(x))(0)
    \end{equation*}
...
\begin{ej}
    \begin{enumerate}
        \item El \textbf{operador traslación} $e^{\lambda T}$ el cual cumple que
        \begin{equation*}
            e^{\lambda T} x^n = \sum_{k=0}^\infty \frac{\lambda^k}{k!}x^n = \sum_{k=0}^\infty \binom{n}{k}y^k x^{n-k} = (x+ \lambda )^n
        \end{equation*} 
        ...
        \end{equation}
        \item El \textbf{operador de Abel} $Te^{\lambda T}$ será
        \begin{equation}\label{operador de Abel}
            Te^{\lambda T}p(x) = p'(x+ \lambda)
        \end{equation}
        \item El operador $\frac{e^{\lambda T} - 1}{T}$ (bien definido ya que $\ord(e^{\lambda T} - 1) = 1$) nos aparecerá posteriormente al tratar con los polinomios de Bernoulli. Este verifica que
        \begin{align*}
...
\begin{definicion}
    Diremos que un operador $F(T) \in \text{Im}(\varphi)$ es un \textbf{operador delta} (o \textbf{$\delta$-operador}) si como serie formal $F(T)$ es una serie delta. Análogamente, diremos $F(T)$ es un \textbf{operador invertible} si como serie formal $F(T)$ es una serie invertible. 
\end{definicion}

\begin{lema}
    Si $F(T)$ es un operador delta y $\Lambda \in \End(\poly)$ conmuta con F, entonces $\gr(\Lambda p(x)) \leq \gr(p(x))$ para todo $p(x) \in \poly$
\end{lema}
\begin{demo}
    ...
    \begin{align*}
        ...
        F(T)x^{0} & = 0
    \end{align*} 
    Por tanto, $\gr(F(T)p(x)) = \gr(p(x)) - 1$ si $\gr(p(x)) \geq 1$. Luego, como $\ord(F^{m+1}(T)) = m+1$ se tendrá que $F^{m+1}(T)x^m = 0$ y en consecuencia
    \begin{equation*}
        F^{m+1}(T) (\Lambda x^m) = \Lambda (F^{m+1}(T)x^m) = 0 
    \end{equation*} 
    Pero $\gr(\Lambda x^n) \geq m+1 \implies \gr(F^{m+1}(T) (\Lambda x^m)) \geq 0 \implies F^{m+1}(T) (\Lambda x^m) \neq 0$, lo cual es imposible. Con lo que $\gr(\Lambda x^n) \leq \gr(x^n)$ y se concluye.
\end{demo}
...
\begin{demo}
    Es claro que los elementos de $\text{Im}(\varphi)$ conmutan con $T$, puesto que conmutan con todas las series formales, luego en particular lo hacen con $T$. Recíprocamente, si $\Lambda$ conmuta con $T$ se verifica que $\Lambda$ tiene la forma de $F(T)$ para la serie formal $F(T)$ definida como:
    \begin{equation*}
    ...
    \end{equation*}
    donde en la segunda igualdad hemos usado que $\binom{n}{k} = \binom{n}{n-k}$, y con esto se concluye 
\end{demo}
