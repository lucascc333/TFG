...
\begin{teorema}
    \textbf{(de existencia de sucesiones de Sheffer)}. Sea $F(T) \in \F^{\delta}$ una serie delta y $G(T) \in \F^*$ una serie invertible. Existe una única sucesion de polinomios $s_n(x)$ con $\gr(s_n) = n$ tal que para todo entero $k \geq 0$ verifica que
...
\begin{demo}
    Ya que $\ord(F) = 1 \implies \ord(F^k) = k$ y $\ord(G) = 0$, tendremos que $\ord(GF^k) = k$. Por tanto, por la proposición \ref{inyectividad sucesion F_k}, la sucesión, si existe, es única. Veamos ahora su existencia. Para ello, construiremos explícitamente la sucesión $\{s_n(x)\}_{n \geq 0}$. 
...
Recordamos que, en el caso del ejemplo anterior, donde $F(T) = T$ y $p_n(x) = x^n$, y en virtud de la proposición \ref{taylor analogo}, tenemos las dos fórmulas 
\begin{equation*}
