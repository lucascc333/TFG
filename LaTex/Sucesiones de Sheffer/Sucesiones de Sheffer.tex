...
Una vez desarrollada toda la teoría de operadores y funcionales lineales sobre el espacio de polinomios, y haber establecido la conexión existente con las series formales, estamos ya en condiciones de estudiar las sucesiones de polinomios. En lo que sigue, y aunque no lo digamos explícitimanente, las sucesiones de polinomios $\{ p_n(x) \}_{n \geq 0}$ serán tan solo aquellas tales que $\gr(p_n) = n$, para todo $n$.

En grandes términos, las sucesiones de Sheffer son un tipo de sucesiones de polinomios que se comportan de una ``forma similar'', respecto a dos series formales concretas, a como lo hacen los polinomios $x^n$ respecto a la serie formal $T$. El teorema más importante en lo que a estas respecta es el siguiente.

\begin{teorema}
    Sea $F(T) \in \F^{\delta}$ una serie delta y $G(T) \in \F^*$ una serie invertible. Existe una única sucesion de polinomios $s_n(x)$ con $\gr(s_n) = n$ tal que
...
\begin{demo}
    ...
    \end{bmatrix}
    \end{equation*}
    Resolviéndolo con métodos clásicos del álgebra lineal obtenemos los coeficientes $c_{n,m}$ y, en consecuencia, los polinomios $s_n(x)$. Por ejemplo, para $n = 0$ obtenemos que $s_0(x) = \frac{1}{a_{0,0}}$ y para $n = 1$
    \begin{equation*}
            \begin{bmatrix}
            a_{1,1} & 0\\
            a_{0,1} & a_{0,0}
    ...
    \end{equation*} 
\end{demo}
de donde vemos que $s_1(x) = \frac{-a_{0,1}}{a_{0,0}a_{1,1}} + \frac{1}{a_{1,1}} x$

\begin{definicion}
    Se llama \textbf{sucesión de Sheffer asociada a $(G(T), F(T))$} a la sucesión de polinomios $s_n(x)$ verificando la condición \eqref{sucesion de sheffer} del teorema anterior. Escribiremos $s_n(x) \sim (G(T), F(T))$
\end{definicion}
\begin{definicion}
    Si $s_n(x) \sim (G(T), T)$ decimos que $s_n(x)$ es la \textbf{sucesión de Appell asociada a $G(T)$}. Análogamente, si $p_n(x) \sim (1, F(T))$ decimos que $p_n(x)$ es simplemente la \textbf{sucesión asociada a $F(T)$}.
\end{definicion}
...
Recordamos que, en el caso del ejemplo anterior, y en virtud de la proposición \ref{taylor analogo}, tenemos las fórmulas 
\begin{align*}
    G(T) & = \sum_{k = 0}^\infty \frac{\langle F(T)| s_k(x)\rangle}{k!} F^k(T)\\
...
%: PROPIEDADES
\section{Propiedades}
Damos aquí algunas propiedades importantes que las sucesiones de Sheffer poseen y que utilizaremos posteriormente.
...
La siguiente propiedad muestra como la sucesiones de Sheffer presentan un comportamiento similar al de la sucesión $x^n$ cuando el producto umbral de dos funcionales actúa sobre ella \eqref{producto funcionales}. Se pone de manifiesto así ese intercambio de exponentes y subíndices del que hablamos en la introducción.
...
\begin{demo}
    Basta con utiliar el teorema anterior en el miembro de la izquierda. 
\end{demo}
...
%: EJEMPLO
\section{La sucesión de factoriales descendentes}
Si bien en el capítulo 4 nos detendremos en algunas sucesiones de polinomios conocidas dentro de la literatura de las matemáticas, conviene que veamos ahora un pequeño ejemplo que muestre cuán útil puede ser todo lo visto hasta ahora.

Consideremos la sucesión de polinomios $p_n(x)$ de los factoriales descendentes; es decir,
\begin{equation*}
    p_n(x) = (x)_n = x(x-1) \cdots (x-n+1)
\end{equation*}

\begin{proposicion}
    La sucesión de factoriales descendentes, $(x)_n$, está asociada al funcional diferencia, $e^T - 1$
\end{proposicion}

\begin{demo}
    Tenemos que
    \begin{align*}
        (e^T - 1)(x)_n = (x+1)_n - (x)_n & = (x+1)x(x-1) \cdots (x+1-(n-1)) - x(x-1) \cdots (x-n+1)\\
        & = nx(x-1) \cdots (x+1-(n-1)) = n(x)_{n-1}
    \end{align*}
    Por \ref{sucesion asociada caracterizacion operador} se obtiene el resultado (pues $\langle 1| (x)_n \rangle = \delta_{n,0}$, de forma evidente).
\end{demo}

Ya que $(e^T - 1)^{-1} = \log(1+T)$ obtenemos por \ref{funcion generadora appell y asociada} que su función generadora es
\begin{equation*}
    \sum_{k=0}^\infty \frac{(x)_k}{k!}T^k = E_x(\log(1+T))
\end{equation*}
Lo cual puede reescribirse para obtener la generalización del teorema binomial.
\begin{teorema}
    Para todo $x \in \field$ se verifica
    \begin{equation*}
        \sum_{k=0}^\infty \binom{x}{k}T^k = (1+T)^x
    \end{equation*}
    donde $\binom{x}{k} = \frac{x(x-1) \cdots (x - k +1)}{k!} = \frac{(x)_k}{k!}$.
\end{teorema}

De la identidad binomial \ref{identidad binomial y de Appell} obtenemos 
\begin{equation*}
    (x+y)_n = \sum_{k=0}^n \binom{n}{k} (x)_k (y)_{n-k}
\end{equation*}

Y de aquí deducimos el teorema, propio de la combinatoria, siguiente.

\begin{teorema}
    \textbf{(fórmula de convolución de Vandermonde)}. Dados $x, y \in \field$ se tiene
    \begin{equation*}
...
\begin{demo}
    Partiendo de la igualdad anterior
    \begin{align*}
        (x+y)_n & = \sum_{k=0}^n \binom{n}{k} (x)_k (y)_{n-k}\\
        \implies \frac{(x+y)_n}{n!} & = \sum_{k=0}^n  \frac{(x)_k}{k!} \cdot \frac{(y)_{n-k}}{(n-k)!}\\
        \implies \binom{x+y}{n} & = \sum_{k=0}^n \binom{x}{k} \binom{y}{n-k}
    \end{align*}
\end{demo}

Por último, a partir de \ref{producto funcionales sucesion asociada y appell} obtenemos una formula para la derivada de los factoriales descendentes en terminos de los mismos.

\begin{proposicion}
    Parar todo $n \geq 1$ se verifica
    \begin{equation*}
        (x)_n' = \sum_{k=1}^{n} \frac{(-1)^{k-1}}{k}(n)_k (x)_{n-k}
    \end{equation*}
\end{proposicion}

\begin{demo}
    Tenemos que, para todo $\lambda \in \field$
    \begin{align*}
        \langle Te^{\lambda T} | (x)_n \rangle & = \sum_{k=0}^n \binom{n}{k} \langle T| (x)_k \rangle \langle e^{\lambda T} | (x)_{n-k} \rangle\\
        \implies (x)_n'(\lambda) & = \sum_{k=0}^n \binom{n}{k} (x)_k'(0) (\lambda)_{n-k}
    \end{align*}
    ...
    \begin{align*}
        (x)_n'(\lambda) & = \sum_{k=1}^n \binom{n}{k} (-1)^{k-1}(k-1)! (\lambda)_{n-k} = \sum_{k=1}^{n} \frac{(-1)^{k-1}}{k}(n)_k (\lambda)_{n-k}
    \end{align*}
    Y como esto es válido para cualquiera $\lambda \in \field$, se concluye. 
\end{demo}
