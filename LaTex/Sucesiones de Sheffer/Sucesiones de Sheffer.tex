...
\begin{demo}
    Ya que $\ord(F(T)) = 1 \implies \ord(F^k(T)) = k$ y $\ord(G(T)) = 0$, tendremos que $\ord(G(T)F^k(T)) = k$. Por tanto, por la proposición \ref{inyectividad sucesion F_k}, la sucesión, si existe, es única. Veamos ahora su existencia. Para ello, construiremos explícitamente esta sucesión de polinomios $s_n(x)$. 
    
    Sean entonces $s_n(x) = \sum_{m=0}^n c_{n,m}x^m \text{ y } G(T)F^k(T) = \sum_{l=k}^\infty a_{k,l}T^l$, donde $c_{n,m}$ son los coeficientes a determinar, es decir, $c_{n,n \neq 0}$, para todo $n \geq 0$, y $a_{k,k} \neq 0$, para todo $k \geq 0$, ya que $\ord(G(T)F^k(T)) = k$. Con todo, tendremos que se debe verificar
    \begin{align*}
...
\begin{corolario}\label{relacion appell y asociada}
    La sucesión de polinomios $s_n(x)$ es la sucesión de Appell asociada a $G(T)$ si y solo si $s_n(x) = \frac{1}{G(T)} x^n$
\end{corolario}
...
\begin{teorema}\label{representacion conjugada}
    ...
    \begin{equation*}
        s_n(x) = \sum_{k=0}^n \frac{1}{k!} \langle \frac{1}{G(F^{-1}(T))}(F^{-1}(T))^k | x^n \rangle x^k
    \end{equation*}
