...
\begin{demo}
    Ya que $\ord(F) = 1$ y $\ord(G) = 0$ tendremos que $\ord(F^k) = k$ y, por tanto, $\ord(GF^k) = k$. Luego, por la proposición \ref{inyectividad sucesion F_k}, la sucesión, si existe, es única. Veamos ahora su existencia. Para ello, construiremos explícitamente la sucesión $\{s_n(x)\}_{n \geq 0}$. 
...
\begin{ej}
    La sucesión $p_n(x) = x^n$ es la sucesión asociada a $F(T) = T$, puesto que sabemos que $\langle F^k(T) | p_n(x) \rangle = \langle T^k | x^n \rangle = n! \delta_{n,k}$
\end{ej}
...
Recordamos que, en el caso del ejemplo anterior, donde $F(T) = T$ y $p_n(x) = x^n$, en virtud de la proposición \ref{taylor analogo} tenemos las dos fórmulas 
\begin{equation*}
...
\end{equation*}
para toda $G(T) \in \F$ y todo $p(x) \in \poly$. Esta observación nos permite enunciar y probar los dos siguientes teoremas clave, generalizaciones de estos hechos y muestras, una vez más, de la relación que esta clase de sucesiones guardan con $p_n(x) = x^n$.
...
\begin{teorema}[función generadora]\label{funcion generadora}
    ...
    \end{equation*}
    para todo $\xi \in \field$ y donde $F^{-1}(T)$ es la inversa con la composición.\footnote{Mantendremos esta notación en todo lo que sigue.} Al miembro de la izquierda se le llama \textbf{función generadora de la sucesión $s_n(x)$}.
\end{teorema}
...
\begin{demo}
    ...
    $\boxed{\impliedby}$ Sea $\overline{s_n}(x)$ asociada a $F(T)$. El operador $\Lambda$ dado por $\Lambda s_n(x) = \overline{s_n}(x)$
    ...
    \begin{equation*}
        F(T) \Lambda s_n(x) = F(T) \overline{s_n}(x) = n\overline{s_{n-1}}(x) = n \Lambda s_{n-1}(x) = \Lambda F(T) s_n(x)
    \end{equation*}
...
\begin{demo}
    ...
    \end{align*}
    Lo que prueba que $E_{y}(T) \circ \Lambda = \Lambda \circ E_{y}(T)$, como queríamos.
\end{demo}
